...
The signed `displayName` remains the only authority that may rename an account-linked player. A room `joinToken` may rejoin/register the same canonical name in another room, but it cannot change that name through `/register`, `/register/:roomId`, `/rooms/:roomId/register`, `/register/rename`, or `/register/:roomId/rename`. A valid, authenticated attempt to do so returns HTTP `403` with `{"ok":false,"reason":"account_display_name_managed","message":"Account-linked display names can only be changed through /identity/exchange"}`. Invalid input, authorization and room state retain their normal earlier `400`, `401`, `404` or `409` responses. Issue a fresh backend-only assertion and call `/identity/exchange` to rename an account-linked player. If the asserted name belongs to another player, the exchange safely retains the account's existing name and returns `displayNameAdjusted: true`.
...
Expired join tokens are now narrower: `/register/me` and rename require a current token. An expired guest token may only refresh the same current display name through registration or a room join; it cannot rename the player or revive an identity after the display name has changed. An account-linked player's expired token can never self-renew through guest registration/join routes. Even with a valid join token, its display name is assertion-managed and cannot change through guest routes; the authenticated UI server must issue a fresh account assertion and call `/identity/exchange` again.
