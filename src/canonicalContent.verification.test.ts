...
function canonicalQuestion(overrides: Record<string, unknown> = {}) {
  return {
    ...
    objectiveId: "objective-foundations-1",
    difficulty: 4,
    difficultyMode: "expert",
    equivalenceKey: "foundations.answer.1",
    ...
    optionExplanations: {
      "option-a": "Alpha is a recognisable but incorrect misconception.",
      "option-b": "Beta is the one best answer.",
      "option-c": "Gamma confuses two related concepts.",
      "option-d": "Delta applies the rule to the wrong subject.",
    },
    optionMisconceptionCodes: {
      "option-a": "FIXTURE_ALPHA",
      "option-c": "FIXTURE_GAMMA",
      "option-d": "FIXTURE_DELTA",
    },
    denominationScope: {},
...
test("protected PostgREST view rows map to the existing four-choice engine format", { concurrency: false }, async () => {
  ...
  try {
    setCanonicalFetchForTests(async () => new Response(JSON.stringify({ data: [{
      ...
      equivalence_key: "mapped.distinction.1",
      ...
      options: [
        { id: "one", position: 1, content: { text: "One" }, is_correct: false, explanation: "One is incorrect.", misconception_code: "ONE" },
        { id: "two", position: 2, content: { text: "Two" }, is_correct: true, explanation: "Two is correct." },
        { id: "three", position: 3, content: { text: "Three" }, is_correct: false, explanation: "Three is incorrect.", misconception_code: "THREE" },
        { id: "four", position: 4, content: { text: "Four" }, is_correct: false, explanation: "Four is incorrect.", misconception_code: "FOUR" },
      ],
    }] }), { status: 200 }));
